...
| `flow_summary.log`   | `Continously updated` | `logs/` | Includes the `INFO`, `ERROR`, `WARNING`, and `SUCCESS` statements outputted by the flow. This should be able to summaries the whole run. |
| `cmds.log`   | `Continously updated` | `./` | Includes all external commands executed by the flow. |
| `config.tcl`   | `Continously updated` | `./` | Includes all configurations used by the run. |
...
| `<design name>.drc.mag`   | `Magic - DRC` | `results/magic` | A MAG file with the DRC error markers but without the violation types generated by magic's DRC engine. This can be generated from a run on either DEF or GDS based on the value of `MAGIC_DRC_USE_GDS`.|
